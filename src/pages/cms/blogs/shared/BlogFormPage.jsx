...
import '../add-blogs/AddBlogPage.css';
...
function BlogFormPage({ mode = 'create' }) {
  ...
  useEffect(() => {
    ...
    const loadCategories = async () => {
      try {
        ...
        setCategoryOptions(toBlogCategorySelectOptions(data));
      } catch (error) {
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    const loadBlog = async () => {
      setLoadingBlog(true);
      try {
        const { response, data } = await getBlogById(id);
        ...
        const detail = extractBlogDetail(data);
        if (!detail) {
          showToast('Blog data not found', 'error');
          navigate('/cms/blogs');
          return;
        }

        const mapped = mapBlogFormFromApi(detail);
  ...
  const handleSubmit = async (status = 'draft') => {
    ...
    try {
      const payload = {
        name: formData.name,
        blog_category_id: formData.category,
  ...
  const breadcrumbs = useMemo(
    () => [
      ...
      { title: 'Blog', link: '/cms/blogs' },
      {
        ...
        link: isEdit ? `/cms/blogs/edit-blog/${id}` : '/cms/blogs/add-blogs',
      },
    ],
    [id, isEdit]
  );
  ...
  if (loadingBlog) {
    return <CommonLoader text="Loading blog..." />;
  }
